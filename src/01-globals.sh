...
if [[ "${GITHUB_REPOSITORY:-}" == "" ]]; then
  ...
  export           USERNAME="$(git remote -v | head -1 | sed 's|.*https://github.com/||;s|.*:||;s|\.git .*||;s/ .*//' | sed 's|\([^/]*\)/\(.*\)|\1|')"
  export          REPOSNAME="$(git remote -v | head -1 | sed 's|.*https://github.com/||;s|.*:||;s|\.git .*||;s/ .*//' | sed 's|\([^/]*\)/\(.*\)|\2|')"
...
export              REPOS_URL="$GITHUB_API_BASEURL/repos/$USERNAME/$REPOSNAME"
